...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   IF (NLP>0 .AND. SOLID_PARTICLES) THEN
      DO IP = 1,NLP
         ...
         ELSE IF (LPC%VEG_WFDS_FE) THEN
           KAPPA_PART(IID,JJD,KKD) = KAPPA_PART(IID,JJD,KKD) + LP%VEG_KAPPA
           KFST4_PART(IID,JJD,KKD) = KFST4_PART(IID,JJD,KKD) + LP%VEG_EMISS
         ENDIF
      ENDDO
   ENDIF
...
IF (SOLID_PARTICLES .AND. UPDATE_INTENSITY) THEN
   PARTICLE_LOOP: DO IP=1,NLP
      ...
      ENDIF
      IF (LPC%VEG_WFDS_FE) THEN
         CALL GET_IJK(LP%X,LP%Y,LP%Z,NM,XID,YJD,ZKD,IID,JJD,KKD)
!        LP%VEG_DIVQR = QR(IID,JJD,KKD)
         LP%VEG_DIVQR = LP%VEG_KAPPA*UII(IID,JJD,KKD) - LP%VEG_EMISS
      ENDIF
   ENDDO PARTICLE_LOOP
ENDIF
