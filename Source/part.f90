...
!Initialize vegetation quanitities for WFDS6 FE model

IF (LPC%VEG_WFDS_FE) THEN 
  LP%VEG_WFDS_FE       = .TRUE.  
  LP%VEG_TMP           = LPC%VEG_INITIAL_TEMPERATURE 
! LPC%TMP_INITIAL      = LPC%VEG_INITIAL_TEMPERATURE
  LP%VEG_FUEL_MASS     = LPC%VEG_BULK_DENSITY
  LP%VEG_MOIST_MASS    = LPC%VEG_MOISTURE*LP%VEG_FUEL_MASS
  LP%VEG_CHAR_MASS     = 0.0_EB
  LP%VEG_ASH_MASS      = 0.0_EB
  LP%VEG_PACKING_RATIO = LPC%VEG_BULK_DENSITY/LPC%VEG_DENSITY 
  LP%VEG_SV            = LPC%VEG_SV 
  LP%VEG_KAPPA         = 0.25*LPC%VEG_SV*LPC%VEG_BULK_DENSITY/LPC%VEG_DENSITY
  LP%VEG_EMISS         = 4._EB*SIGMA*LP%VEG_KAPPA*LP%ONE_D%TMP_F**4
  LP%VEG_DIVQR         = 0.0_EB
  RETURN
ENDIF
...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   TIME_STEP_LOOP: DO ITER=1,N_ITER
      ...
      IF(LPC%VEG_WFDS_FE) R_D = 2._EB/LP%VEG_SV
...
DRAG_LAW_SELECT: SELECT CASE (LPC%DRAG_LAW)
   ...
   CASE(VEG_WFDS_FE_DRAG) !Same as DEFAULT
      ...
      C_DRAG = LPC%VEG_VOLUME_FRACTION*DRAG(LP%RE,2) !cylinder
...
SPECIES_LOOP: DO Z_INDEX = 1,N_TRACKED_SPECIES
   ...
   FILM_SUMMING_LOOP: DO IP=1,NLP
      LP  => LAGRANGIAN_PARTICLE(IP)
      LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
      IF (LPC%VEG_WFDS_FE) CYCLE FILM_SUMMING_LOOP
...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   WEED_LOOP: DO
      ...
      ! Remove WFDS6 veg particles
      IF (LPC%VEG_WFDS_FE .AND. LP%ONE_D%BURNAWAY) THEN
         CALL PARTICLE_ORPHANAGE
         CYCLE WEED_LOOP
      ENDIF
