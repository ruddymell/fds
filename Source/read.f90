...
REAL(EB) :: VEG_SV,VEG_MOISTURE,VEG_CHAR_FRACTION,VEG_DRAG_CONSTANT,VEG_DENSITY,VEG_BULK_DENSITY, &
            ...
            VEG_FUEL_MPV_MIN,VEG_MOIST_MPV_MIN,VEG_CHAR_ENTHALPY_FRACTION,VEG_ASH_FRACTION,VEG_VOLUME_FRACTION
...
LOGICAL :: VEG_REMOVE_CHARRED,VEG_STEM,VEG_CHAR_OXIDATION,VEG_IGNITED,VEG_HCONV_CYLLAM,VEG_HCONV_CYLRE,VEG_HCONV_CYLMAX, &
           VEG_HCONV_CYLMIXED,VEG_WFDS_FE,VEG_CHAR_OX_CO2GAIN_O2LOSS,VEG_CHAR_OX_GRISHIN
...
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                VEG_STEM,VEG_SV,&
                VEG_CHAR_ENTHALPY_FRACTION,VEG_ASH_FRACTION, & 
                VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
                VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_CHAROX_RATE_MAX,VEG_HCONV_CYLLAM,VEG_HCONV_CYLRE,VEG_HCONV_CYLMAX, &
                VEG_HCONV_CYLMIXED,VEG_WFDS_FE,VEG_VOLUME_FRACTION,VEG_CHAR_OX_CO2GAIN_O2LOSS,VEG_CHAR_OX_GRISHIN
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   LPC%VEG_WFDS_FE                      = VEG_WFDS_FE
   LPC%VEG_SV                           = VEG_SV !1/m
   LPC%VEG_VOLUME_FRACTION              = VEG_VOLUME_FRACTION
   LPC%ID                               = ID
   LPC%BREAKUP                          = BREAKUP
   ...
   IF (LPC%LIQUID_DROPLET) THEN !Set KILL_RADIUS for SURF_ID in PROC_PART
      ...
      ENDIF
   ENDIF
   IF(LPC%VEG_WFDS_FE) MINIMUM_DIAMETER = 4._EB/LPC%VEG_SV !cylindrical veg element
   ...
   ! WFDS6 Fuel Element vegetation properties

   LPC%VEG_MOISTURE             = VEG_MOISTURE
   LPC%VEG_CHAR_FRACTION        = VEG_CHAR_FRACTION
   LPC%VEG_ASH_FRACTION         = VEG_ASH_FRACTION !fraction of original dry fuel mass
   LPC%VEG_DRAG_CONSTANT        = VEG_DRAG_CONSTANT   
   LPC%VEG_HCONV_CYLLAM         = VEG_HCONV_CYLLAM
   LPC%VEG_HCONV_CYLRE          = VEG_HCONV_CYLRE 
   LPC%VEG_HCONV_CYLMAX         = VEG_HCONV_CYLMAX
   LPC%VEG_HCONV_CYLMIXED       = VEG_HCONV_CYLMIXED
   ...
   LPC%VEG_CHAROX_RATE_MAX      = VEG_CHAROX_RATE_MAX !kg/m3/s
   LPC%VEG_DEHYDRATION_RATE_MAX = VEG_DEHYDRATION_RATE_MAX !kg/m3/s
   LPC%VEG_INITIAL_TEMPERATURE  = VEG_INITIAL_TEMPERATURE +TMPM ! K
!  LPC%VEG_FUEL_MPV_MIN         = 0.001_EB*VEG_BULK_DENSITY + (1._EB-VEG_VOLUME_FRACTION)*VEG_BULK_DENSITY
   LPC%VEG_FUEL_MPV_MIN         = 0.001_EB*VEG_BULK_DENSITY
   LPC%VEG_MOIST_MPV_MIN        = VEG_MOISTURE*LPC%VEG_FUEL_MPV_MIN
   ...
   LPC%VEG_CHAR_ENTHALPY_FRACTION = VEG_CHAR_ENTHALPY_FRACTION
   LPC%VEG_ASH_MPV_MAX          = VEG_ASH_FRACTION*VEG_BULK_DENSITY
   LPC%VEG_H_H2O                = VEG_H_H2O*1000._EB !J/kg
   LPC%VEG_A_H2O                = VEG_A_H2O 
   LPC%VEG_E_H2O                = VEG_E_H2O
   LPC%VEG_H_PYR                = VEG_H_PYR*1000._EB !J/kg 
   LPC%VEG_A_PYR                = VEG_A_PYR 
   LPC%VEG_E_PYR                = VEG_E_PYR 
   LPC%VEG_H_CHAR               = VEG_H_CHAR*1000._EB !J/kg 
   LPC%VEG_A_CHAR               = VEG_A_CHAR
   LPC%VEG_E_CHAR               = VEG_E_CHAR 
   LPC%VEG_BETA_CHAR            = VEG_BETA_CHAR
   LPC%VEG_NU_O2_CHAR           = VEG_NU_O2_CHAR 
   LPC%VEG_CHAR_OX_CO2GAIN_O2LOSS = VEG_CHAR_OX_CO2GAIN_O2LOSS
   LPC%VEG_CHAR_OX_GRISHIN        = VEG_CHAR_OX_GRISHIN 
   ...
   SELECT CASE(DRAG_LAW)
      ...
      CASE DEFAULT
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
      CASE('VEG WFDS FE DRAG')
         LPC%DRAG_LAW = VEG_WFDS_FE_DRAG
   END SELECT
...
!WFDS6 Fuel element parameter defaults
VEG_SV                   = 4000. ! 1/m
VEG_MOISTURE             = 1.0_EB
...
VEG_HCONV_CYLMAX         = .TRUE.
VEG_HCONV_CYLMIXED       = .FALSE.
...
VEG_WFDS_FE              = .FALSE.
VEG_VOLUME_FRACTION      = 1.0_EB
VEG_CHAR_OX_CO2GAIN_O2LOSS = .FALSE. !include CO2 production and O2 (maybe) consumption during char oxidation
VEG_CHAR_OX_GRISHIN        = .FALSE. !use Grishin's char oxidation Arrhenius formula
...
NAMELIST /SURF/ ADIABATIC,BACKING,BLOWING,BURN_AWAY,BURN_DURATION,CELL_SIZE_FACTOR,&
                ...
                WFDS_BF_VEG,VEG_DRAG_CONSTANT,VEG_HCONV_CYLRE,VEG_HCONV_CYLLAM,FIRELINE_MLR_MAX, &
                VEG_HCONV_CYLMAX,VEG_CHAR_OXIDATION,VEG_ASH_FRACTION,VEG_CHAR_FRACTION,VEG_DENSITY,VEG_GROUND_TEMP,VEG_HEIGHT,&
                VEG_H_H2O,VEG_A_H2O,VEG_E_H2O,VEG_H_PYR,VEG_A_PYR,VEG_E_PYR,VEG_H_CHAR,VEG_A_CHAR,VEG_E_CHAR, &
                VEG_BETA_CHAR,VEG_NU_O2_CHAR,VEG_CHAR_ENTHALPY_FRACTION, &
                VEG_INITIAL_TEMP,VEGETATION_LAYERS,VEG_LOAD,VEG_REMOVE_CHARRED,VEG_REMOVE_ASH,VEG_UNIT_DRAG_COEFF, &
                VEG_MOISTURE,VEG_NO_BURN,VEG_POSTFIRE_DRAG_FCTR,VEG_SV,VEG_DEGRADATION,VEG_MLRPUVMAX,VEG_WFDS_FE, &
...
VEGETATION_SURF_INDEX        = N_SURF + 11
...
SURFACE(VEGETATION_SURF_INDEX)%ID        = 'WFDS BF VEG' !WFDS boundary fuel
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   ! Boundary Fuel Vegetation specific

   IF (WFDS_BF_VEG) WFDS_BNDRYFUEL = .TRUE.
   SF%VEG_WFDS_FE        = VEG_WFDS_FE
   SF%WFDS_BF_VEG        = WFDS_BF_VEG !T or F
   SF%VEG_NO_BURN        = VEG_NO_BURN
   SF%VEG_ASH_FRACTION   = VEG_ASH_FRACTION
   SF%VEG_CHAR_FRACTION  = VEG_CHAR_FRACTION
   SF%VEG_DENSITY        = VEG_DENSITY
   SF%VEG_MOISTURE       = VEG_MOISTURE
   SF%VEG_HEIGHT         = VEG_HEIGHT
   SF%VEG_INITIAL_TEMP   = VEG_INITIAL_TEMP
   SF%VEG_GROUND_TEMP    = VEG_GROUND_TEMP
   IF (VEG_GROUND_TEMP  /= -99._EB) SF%VEG_GROUND_ZERO_RAD = .FALSE.
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   SF%NVEG_L             = INT(1._EB + VEG_HEIGHT*3._EB*SF%VEG_KAPPA)
   ...
   SF%VEG_H_PYR          = VEG_H_PYR*1000._EB !J/kg/K
   SF%VEG_A_PYR          = VEG_A_PYR
   SF%VEG_E_PYR          = VEG_E_PYR
   SF%VEG_H_CHAR         = VEG_H_CHAR*1000._EB !J/kg/K
   ...
   SF%VEG_REMOVE_CHARRED = VEG_REMOVE_CHARRED !for particle model
   SF%VEG_REMOVE_ASH     = VEG_REMOVE_ASH     !for particle model
   SF%VEG_UNIT_DRAG_COEFF= VEG_UNIT_DRAG_COEFF !for BF model
   ...
   !Particle vegetation specific

   SF%VEG_MLRPUVMAX      = VEG_MLRPUVMAX
...
!Boundary fuel vegetation parameter default values
WFDS_BF_VEG                  = .FALSE.
VEG_NO_BURN                  = .FALSE.
VEG_DRAG_CONSTANT            = 1.0_EB
VEG_ASH_FRACTION             = 0.03_EB !fraction of original dry fuel mass that can become ash
VEG_CHAR_FRACTION            = 0.20_EB !fraction of original dry fuel mass that can become char
VEG_DENSITY                  = 512._EB !kg/m^3 density of dry fuel particle
VEG_HEIGHT                   = 0.50_EB !m
VEG_INITIAL_TEMP             = TMPA-TMPM
VEG_GROUND_TEMP              = -99._EB
...
VEG_HCONV_CYLLAM             = .TRUE.  !cylindrical laminar convection heat transfer
VEG_HCONV_CYLMAX             = .FALSE. !maximum of cylindrical free (Morgan) vs. forced (Hilpert) convection heat transfer
VEG_POSTFIRE_DRAG_FCTR       = 0.0_EB !Fraction of initial drag remiaing after veg is burned away

VEG_DEGRADATION            = 'LINEAR'
VEG_CHAR_OXIDATION         = .FALSE.
VEG_H_H2O                  = 2259._EB !J/kg/K heat of water evaporation
VEG_A_H2O                  = 600000._EB !1/s sqrt(K)
VEG_E_H2O                  = 5800._EB !K
VEG_H_PYR                  = 418._EB !J/kg/K heat of pyrolysis Porterie, Grishin
VEG_A_PYR                  = 36300._EB !1/2 Porterie, Grishin
VEG_E_PYR                  = 7250._EB !K Porterie, Grishin
VEG_H_CHAR                 = -12.0E+3_EB !kJ/kg/K Porterie, Grishin
VEG_A_CHAR                 = 430._EB !m/s Porterie, Grishin
VEG_E_CHAR                 = 9000._EB !K Porterie, Grishin
VEG_BETA_CHAR              = 0.2_EB !Porterie
VEG_NU_O2_CHAR             = 1.65_EB !Porterie
VEG_CHAR_ENTHALPY_FRACTION = 0.5_EB !Porterie
VEG_MLRPUVMAX              = 9999._EB !default bound on massloss rate per unit volume for a veg represented as particles
VEG_REMOVE_CHARRED         = .FALSE.
VEG_REMOVE_ASH             = .FALSE.
VEG_UNIT_DRAG_COEFF        = .TRUE. !A constant empirical drag coefficient = 1 is assumed, no Re dependence
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (SF%WFDS_BF_VEG) SF%SPECIES_BC_INDEX = SPECIFIED_MASS_FLUX
   ! Texture map info
   ...
   ! Boundary fuel surface vegetation
   ...
   DO WHILE (FINISH .EQ. 0)
      DO KK = 1, NK2
         ...
         DO JJ = 1, NJ2
            J = NJ_LIST(JJ)
            INDEX = (K-1)*NJ + J
            IF(MULTI_RES_ORDER(INDEX) .EQ. 0) THEN
               MULTI_RES_ORDER(INDEX) = COUNT
               COUNT = COUNT + 1
            ENDIF
         END DO
      END DO
