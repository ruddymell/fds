...
! Vegetation surface drag for WFDS Boundary Fuel

ALLOCATE(M%VEG_DRAG(0:IBP1,0:JBP1,0:10),STAT=IZERO) ; CALL ChkMemErr('INIT','VEG_DRAG',IZERO)
M%VEG_DRAG = 0._EB
...
! Set clock for BF and FE WFDS vegetation models

M%VEG_CLOCK_BC = T_BEGIN
M%VEG_CLOCK_FE = T_BEGIN
...
IF (SF%WFDS_BF_VEG.AND. .NOT.EVACUATION_ONLY(NM)) THEN

   WC%ONE_D%VEG_FUELMASS_L(:)  = SF%VEG_LOAD / SF%NVEG_L
   WC%ONE_D%VEG_MOISTMASS_L(:) = SF%VEG_MOISTURE*WC%ONE_D%VEG_FUELMASS_L(:)
