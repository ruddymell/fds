      ...
      DO NN=1,LPC%N_QUANTITIES
         SELECT CASE(LPC%QUANTITIES_INDEX(NN))
            ...
            CASE(437)  ! PARTICLE TEMPERATURE
               ...
               IF(LP%VEG_WFDS_FE) QP(NPP,NN) = LP%VEG_TMP - TMPM
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE( 5) ! WALL TEMPERATURE
      ...
      IF(PART_INDEX > 0) THEN
        IF(LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)%VEG_WFDS_FE) SOLID_PHASE_OUTPUT = LP%VEG_TMP - TMPM
      ENDIF
   ...
   CASE(24) ! WALL THICKNESS
      ...
      IF (SURFACE(WC%SURF_INDEX)%WFDS_BF_VEG) THEN     !surface vegetation height for boundary fuel model
         SOLID_PHASE_OUTPUT = ONE_D%VEG_HEIGHT
      ENDIF
